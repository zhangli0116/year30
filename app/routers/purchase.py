...
def create_purchase(
    ...
    return success(crud.purchase.create_purchase(db, payload))
...
def create_purchases_batch(
    ...
    return success(crud.purchase.create_purchases(db, payload))
...
def update_purchase(
    ...
    if payload.fund_id is not None and crud.fund.get_fund(db, payload.fund_id) is None:
        return error(40003, f"基金 {payload.fund_id} 不存在")
    return success(crud.purchase.update_purchase(db, record, payload))
